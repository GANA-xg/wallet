...
export const physicalCards = pgTable(
  ...
  (table) => ({
    walletIdx: index("idx_physical_cards_wallet").on(table.walletId),
    statusIdx: index("idx_physical_cards_status").on(table.status),
    expiryIdx: index("idx_physical_cards_expiry").on(table.expiryYear, table.expiryMonth),
  }),
);

export const insertPhysicalCardSchema = createInsertSchema(physicalCards, {
  ...
  status: z
    ...
    .default("active"),
  isPinSet: z.boolean().default(false),
...
export const selectPhysicalCardSchema = createSelectSchema(physicalCards);
